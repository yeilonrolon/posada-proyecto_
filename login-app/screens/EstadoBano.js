...
export default function EstadoBano({ navigation, route }) {
    ...
    const verificarAntiguedad = useCallback((datosServidor) => {
        ...
        // Alerta si existen registros vencidos y el rol NO es administrador
    ...
    const cargarDatos = useCallback(async () => {
        try {
            // CORRECCIÓN AXIOS: Se quita el objeto vacío {} intermedio para que el timeout funcione
            ...
            try {
                // CORRECCIÓN AXIOS: Configuración directa como segundo parámetro
    ...
    const renderBano = ({ item }) => (
        <View style={styles.card}>
            <View style={styles.cardHeader}>
                <Text style={styles.habitacionTexto}> Habitación N°{item.num_habitacion}</Text>
                <Text style={styles.fechaTexto}>{item.fecha_lista}</Text>
            </View>
            <View style={styles.divisor} />
            <View style={styles.cardBody}>
                <Text style={styles.estadoTexto}>{item.estado} </Text>
                {/* CORRECCIÓN: Se actualizó el estilo asignado para coincidir con el StyleSheet */}
    ...
    return (
        <View style={styles.contenedorPrincipal}>
            ...
            {/* Renderizado condicional del botón de creación basado en el rol */}
            {rol !== "Admin" && (
                <TouchableOpacity 
...
const styles = StyleSheet.create({
    ...
    observacionTexto: { fontSize: 16, color: '#334155', marginTop: 4 },
